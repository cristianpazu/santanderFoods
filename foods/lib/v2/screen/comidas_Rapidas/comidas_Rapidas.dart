...
import 'package:foods/v2/screen/comidas_Rapidas/Menu_Comidas_Rapidas.dart';
...
class _ComidasRapidasState extends ConsumerState {
  ...
  Widget build(BuildContext context) {
    ...
    final todasLasComidas =
        comidaRapida.comidaRapida.expand((e) => e.nombreComidaRapida).toList();
                  ...
                  children: [
                    ...
                    Padding(
                      padding: const EdgeInsets.only(
                          left:
                              16.0), // margen izquierdo para separar del borde
                      child: SingleChildScrollView(
                        scrollDirection: Axis.horizontal,
                        child: Row(
                          children: comidas
                              .map((comida) => Padding(
                                    padding: const EdgeInsets.only(
                                        right: 12.0), // Espacio entre tarjetas
                                    child: cardComidaRapidasWidget(
                                      colors: Colors.white,
                                      image: Image.asset(comida.image,
                                          fit: BoxFit.contain),
                                      texto: comida.nombres,
                                      redireccionamiento: MenucomidasRapidas(
              idRestaurante: comida.id,
            ),
                                    ),
                                  ))
                              .toList(),
                    ...
                    const SizedBox(
                        height: 16.0), // Espacio inferior entre categorías
                  ],
                );
              }).toList(),
            ),
