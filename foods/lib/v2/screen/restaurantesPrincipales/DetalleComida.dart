  ...
  children: [
    ...
    Container(
      ...
      child: GridView.count(
        physics: NeverScrollableScrollPhysics(), // para que no se pueda hacer scroll dentro del Grid
        ...
        children: widget.salsas!.map((salsa) {
          ...
          return CheckboxListTile(
           activeColor:Colors.white, // color del check cuando está activo
        checkColor:  Color(ConstantesColorTema.fondoColorAppbar),
            contentPadding: EdgeInsets.zero,
            title: UiTexto(texto: nombre).textoRobotoLight6(), //Text(nombre),
            value: _salsasSeleccionadas[id] ?? false,
            onChanged: (selected) {
              final yaSeleccionada = _salsasSeleccionadas[id] ?? false;
              if (!yaSeleccionada && cantidadSeleccionadas >= 2) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Solo puedes seleccionar hasta 2 salsas'),
